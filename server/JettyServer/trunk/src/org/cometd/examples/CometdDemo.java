...
public class CometdDemo {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * ssl_connector
		 * .setKeystore(base+"/examples/src/test/resources/keystore");
		 * ssl_connector.setPassword("OBF:1vny1zlo1x8e1vnw1vn61x8g1zlu1vn4");
		 * ssl_connector.setKeyPassword("OBF:1u2u1wml1z7s1z7a1wnl1u2g");
		 * ssl_connector
		 * .setTruststore(base+"/examples/src/test/resources/keystore");
		 * ssl_connector
		 * .setTrustPassword("OBF:1vny1zlo1x8e1vnw1vn61x8g1zlu1vn4");
		...
		// contexts.addHandler(resource_handler);
		...
		context.setBaseResource(new ResourceCollection(
				new Resource[] { Resource.newResource("webapp/"),
		...
		ServletHolder dftServlet = context
				.addServlet(DefaultServlet.class, "/"); // not show error 404,
														// show directory
		dftServlet.setInitOrder(1);
		dftServlet.setInitParameter("dirAllowed", "true");
		...
		comet.setInitParameter("transports",
				"org.cometd.server.websocket.WebSocketTransport");
		...
		if (Boolean.getBoolean("LAZY")) {
			bayeux.addExtension(new BayeuxServer.Extension() {
				...
				public boolean rcv(ServerSession from, Mutable message) {
					if (message.getChannel().startsWith("/chat/")
							&& message.getData() != null
							&& message.getData().toString().indexOf("lazy") >= 0)
